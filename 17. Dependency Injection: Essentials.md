...
interface GitHub {
  getIssues: (organization: string) => Effect<Chunk.Chunk<Issue>>
  postComment: (issue: Issue, comment: Comment) => Effect<void>
}

interface Http {
  get: (url: string) => Effect<Chunk.Chunk<number>>
  post: (url: string, body: Chunk.Chunk<number>) => Effect<Chunk.Chunk<number>>
}
